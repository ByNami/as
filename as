...
function checkLabel(label)
    if label:IsA("TextLabel") then
        if label.Text == LocalPlayer.Name then
            label.Text = "With ❤️ Nami Hub"
        end
...
Tabs = {
    Infor = Window:AddTab({ Title = "• Infor", Icon = "rbxassetid://76424412980959" }),
    Player = Window:AddTab({Title = "• Player", Icon = "rbxassetid://79457684163404"}),
    Autofarm = Window:AddTab({ Title = "• Auto Farm", Icon = "rbxassetid://18831441803" }),
    Hatch = Window:AddTab({Title = "• Hatch", Icon = "rbxassetid://18319580325"}),
    Roll = Window:AddTab({Title = "• Roll", Icon = "rbxassetid://18786832523"}),
    Dungeon = Window:AddTab({Title = "• GameModes", Icon = "rbxassetid://138293173884000"}),
    Exchange = Window:AddTab({Title = "• Exchange", Icon = "rbxassetid://101233904135267"}),
    Misc = Window:AddTab({ Title = "• Misc", Icon = "rbxassetid://81728873011371" }),
...
function updateLiveStatus()
    if isUpdatingStatus then return end
...
function processRollQueue()
    if currentlyRolling then return end
    ...
    local noTokensCounter = 0
    local maxNoTokensWait = 10
        ...
        if config then
            ...
            if tokens < config.tokensRequired then
                ...
                if not foundAlternative then
                    ...
                    noTokensCounter = noTokensCounter + 1
                    
                    if noTokensCounter >= maxNoTokensWait then
                        task.wait(2.0)
                    else
                        task.wait(1.0)
                    end
